...
cat("Number of publications in wos3: ", nrow(M_wos3))
```
...
```{r}
M_wos3t<-M_wos3[order(M_wos3$SC),]
scgroup<-tapply(M_wos3t$SC, M_wos3t$SC, function(x) seq(1, length(x),1))
M_wos3t$scgroup<-unlist(scgroup)

summarise(M_wos3t$scgroup)
```
...
M_wos3 %>%
  ...
  group_by(WC_list) %>%
  summarise(WC_n = n(),
            WC_n_frac = (n() / mean(n_doc)) %>% round(0) ) %>%
  mutate(WC_share = (WC_n / n()) %>% round(2),
         WC_share_frac = (WC_n_frac / sum(WC_n_frac)) %>% round(2) ) %>%
    arrange(desc(WC_share))
```

## Most frequent journals per top 10 WoS subject-categories

```{r}
M_wos3 %>%
  mutate(WC_list = WC %>% str_split("; ")) %>%
  select(UT, JI, WC_list) %>%
  unnest(JI) %>%
  group_by(UT) %>%
  mutate(n_j = n()) %>%
  ungroup() %>%
  group_by(JI, WC) %>%
  summarise(JI_n = n()) %>%
  mutate(JI_share = (JI_n / n()) %>% round(2) ) %>%
    arrange(desc(JI_share))
```
...
options(width=100)
S_wos2 <- summary(object = results_wos2$SC, k = 10, pause = FALSE)
```
...
```{r}
results_wos3<-biblioAnalysis(M_wos3, sep=";")
options(width=100)
...
```{r}
CR_wos3 <- citations(M_wos3, field = "article", sep = ";")
#cbind(CR_wos2$Cited[1:10])
...
```{r}
CR_wos3$Cited %>% as_tibble() %>% head(20) %>% 
  kable() %>%
  kable_styling(bootstrap_options = c("striped", "hover", "condensed", "responsive"), font_size = 8)
```
...
```{r}
CR_wos3 <- citations(M_wos3, field = "author", sep = ";")
cbind(CR_wos3$Cited[1:10])

```

```{r}

CR_wos3 <- localCitations(M_wos3, sep = ";")
CR_wos3$Authors[1:10,]
CR_wos3$Papers[1:10,]
...
NetMatrix_wos3 <- biblioNetwork(M_wos3, analysis = "co-occurrences", network = "keywords", sep = ";")

net_wos3=networkPlot(NetMatrix_wos3, normalize="association", weighted=T, n = 30, Title = "Keyword Co-occurrences", type = "fruchterman", size=T,edgesize = 5,labelsize=0.7)
...
Map_wos3=thematicMap(M_wos3, field = "ID", n = 250, minfreq = 5,
  stemming = FALSE, size = 0.5, n.labels=3, repel = TRUE)
plot(Map_wos3$map)
