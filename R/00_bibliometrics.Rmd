...
M_wos3 %>%
  ...
  unnest(WC_list) %>%
  count(., WC_list, sort = TRUE) %>% slice(1:10) %>% pull(WC_list)
```
...
options(width=100)
S_wos3 <- summary(object = results_wos3, k = 20, pause = FALSE)
```
...
```{r}
results_wos3<-biblioAnalysis(M_wos3_first, sep=";")
options(width=100)
S_wos3 <- summary(object = results_wos3, k = 10, pause = FALSE)
```

```{r}
M_wos3_second<- M_wos3 %>%
filter(PY >= 2010 & PY <= 2019) %>%
  filter(DT=="ARTICLE") %>%
  as.data.frame()
```
...
```{r}
results_wos3<-biblioAnalysis(M_wos3_second, sep=";")
options(width=100)
S_wos3 <- summary(object = results_wos3, k = 10, pause = FALSE)
```
